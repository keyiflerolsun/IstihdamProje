...
taban = KekikTaban(
    baslik   = "@keyiflerolsun Istihdam",
    aciklama = "Istihdam Başlatıldı..",
    banner   = "Istihdam",
    girinti  = 6
)
...
if MongoDB:
    from Lib.DB._MongoDB import FlaskAPIDB
else:
    from Lib.DB._TinyDB import FlaskAPIDB
...
def istek_log(yanit:Response) -> Response:
    ...
    log_veri = {
        'id'     : session.get('kullanici_adi') or '', 
...
####
from FlaskAPI._endpointler import _hata, ana_sayfa
